...
export function useHabitStatistics() {
	...
	const { data: logs = [] } = useAllHabitLogs(
	...
	return useQuery({
		...
		queryFn: () => {
			...
			const uniqueDaysWithLogs = new Set(
				completedLogs.map((log) => format(parseISO(log.date), "yyyy-MM-dd"))
			).size;
			...
			for (let i = 3; i >= 0; i--) {
				...
				weeklyData.push({
					...
					rate:
						totalHabits > 0
							? Math.round((weekLogs.length / (7 * totalHabits)) * 100)
							: 0,
				});
			}
			...
			const habitPerformance = activeHabits
				.map((habit) => {
					const habitLogs = completedLogs.filter(
						(log) => log.habit_id === habit.id
					);
					const habitDays = new Set(
						habitLogs.map((log) => format(parseISO(log.date), "yyyy-MM-dd"))
					).size;
					...
					return {
						...
						lastCompleted:
							habitLogs.length > 0
								? format(
										parseISO(habitLogs[habitLogs.length - 1].date),
										"MMM d"
								  )
								: "Never",
					};
			...
			activeHabits.forEach((habit) => {
				if (habit.category) {
					...
					categoryStats[categoryName].completions += completedLogs.filter(
						(log) => log.habit_id === habit.id
					).length;
				}
			});
			...
			const categoryPerformance = Object.values(categoryStats).map((cat) => ({
				...
				rate:
					cat.totalPossible > 0
						? Math.round((cat.completions / cat.totalPossible) * 100)
						: 0,
			}));
			...
			return {
				overview: {
					...
					averagePerDay:
						uniqueDaysWithLogs > 0
							? Math.round(totalCompletions / uniqueDaysWithLogs)
							: 0,
				},
				trends: {
					weekly: weeklyData,
